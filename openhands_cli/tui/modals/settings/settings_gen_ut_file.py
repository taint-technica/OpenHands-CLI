...
class GenUnitTestFileSettings(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            yield Label(
                'File Name, for etc. "src/main/java/com/example/Greeting.java" or "backend/controller/api/routes/ranking.py"'
            )
            yield Input(
                value=self.src_file_name,
                placeholder="Enter source file name...",
                id="src_file_name",
            )
            yield Label("Coverage expectation % (1-100)")
            yield Input(
                value=self.coverage_expect,
                placeholder="Enter coverage expectation in percentage...",
                id="coverage_expect",
            )
            yield Label("Number of iteration")
            yield Input(
                value=self.num_iteration,
                placeholder="Enter number of iteration...",
                id="num_iteration",
            )
            ...
            self.dismiss(
                {
                    ...
                    "coverage_expect": coverage_expect,
