...
class SonarScannerSettings(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            ...
            yield Input(
                value=self.inclusive_path,
                placeholder="Enter source path...",
                id="inclusive_path",
            )
            yield Label("Exclusive path")
            yield Input(
                value=self.exclusive_path,
                placeholder="Enter exclusive source path...",
                id="exclusive_path",
            )
            ...
            self.dismiss(
                {
                    ...
                    "inclusive_path": inc_path,
                    "exclusive_path": excl_path,
                }
            )  # Pass back with updated values
        else:
