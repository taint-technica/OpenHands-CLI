...
## 🔄 Cập Nhật: Bổ Sung Refactor Guide Sau Khi Generate Tests
...
Sau khi Keploy generate tests, agent sẽ refactor file test để:
- ✅ Clean code, rõ ràng
- ✅ Tuân thủ best practices cho Python/Java
- ✅ Loại bỏ code thừa, comments placeholder
- ✅ Đảm bảo tests chất lượng cao, dễ bảo trì
...
```
openhands_cli/instructions/utgen/
├── __init__.py              # SỬA: Export refactor guides
├── constants.py             # Không thay đổi
├── workflows.py             # SỬA: Thêm Step 9 - Refactor Tests
├── error_handling.py        # Không thay đổi
└── refactor_guides.py       # TẠO MỚI: Chứa hướng dẫn refactor
```
...
#### **1. TẠO MỚI: `openhands_cli/instructions/utgen/refactor_guides.py`**

File này chứa hướng dẫn refactor chi tiết cho Python và Java:
...
After Keploy generates tests, refactor the test file to ensure clean, maintainable code.

### 1. Follow AAA Pattern Strictly

Every test function MUST follow the Arrange-Act-Assert pattern:

```python
def test_should_raise_not_found_when_user_id_does_not_exist():
    # Arrange
...
**Format:** `test_should_[expected_result]_when_[condition]`
...
def test_should_return_user_when_id_is_valid(user_service, mock_user_repo):
    # Arrange
    user_id = "123"
...
def test_should_validate_email_correctly(email, expected):
    # Arrange & Act
...
class TestUserService:
    ...
    def test_should_return_user_when_id_is_valid(self):
        pass
    
    def test_should_raise_not_found_when_user_does_not_exist(self):
        pass

class TestUserServiceWithInvalidInput:
    """Tests for edge cases and invalid inputs."""
    
    def test_should_throw_when_id_is_none(self):
        pass
    
    def test_should_throw_when_id_is_empty(self):
        pass
```

### 6. Use conftest.py for Shared Fixtures

Create `tests/conftest.py` for fixtures used across multiple test files:
...
**Use MagicMock/patch as context manager or decorator:**
...
def test_should_use_mock(mock_api):
    mock_api.return_value = {"status": "ok"}
    # Test code

# As context manager
def test_should_use_context_manager():
    with patch('module.external_api_call') as mock_api:
        mock_api.return_value = {"status": "ok"}
        # Test code

# Using pytest-mock
def test_should_use_mocker(mocker):
...
✅ **Good:**
```python
def test_should_return_user_when_id_is_valid():
    pass

def test_should_raise_not_found_when_id_is_invalid():
    pass
```
...
def test_user_operations():
    # Tests 5 different things - too long!
    pass
```

### 9. Use Descriptive Docstrings

For complex test logic, add docstrings:

```python
def test_should_handle_concurrent_updates():
    """
    Verify that concurrent updates to the same record
    are handled correctly using optimistic locking.
    
    Scenario:
    1. Two users fetch the same record
    2. Both modify and save
    3. Second save should fail with ConflictError
    """
    pass
```
...
- Hit real databases, APIs, or file systems — mock everything
- Use `unittest.TestCase` style — use native pytest only
- Test private methods (prefixed with `__`) directly
- Leave placeholder comments like `# add more tests`
...
- Test via public interface only
...
void Test_should_ThrowNotFoundException_when_UserIdDoesNotExist() {
    // Arrange
...
**Format:** `Test_should_[ExpectedResult]_when_[Condition]`
...
### 3. Use MockitoExtension for Service Layer Tests
...
class UserServiceTest {
    ...
    @Test
    void Test_should_ReturnUser_when_IdIsValid() {
        // Test code
    }
}
```
...
void Test_should_ValidateEmailCorrectly(String email, boolean expected) {
    // Arrange & Act
...
void Test_should_CreateUser_when_DataIsValid(User user, boolean expected) {
    // Test code
}
...
### 5. Use @Nested Classes to Group Related Scenarios
...
class UserServiceTest {
    ...
    @Nested
    @DisplayName("Tests for getUser method")
    class GetUserTests {
        ...
        void Test_should_ThrowException_when_UserNotFound() {
            // Test code
        }
    }
    ...
    class CreateUserTests {
        ...
        void Test_should_CreateUser_when_DataIsValid() {
            // Test code
        }
    }
}
...
### 6. Use @DisplayName for Human-Readable Descriptions

```java
@Test
@DisplayName("Should throw IllegalArgumentException when email is null")
void Test_should_ThrowIllegalArgumentException_when_EmailIsNull() {
    // Test code
}
```

### 7. Use AssertJ Instead of JUnit assertEquals

**Prefer AssertJ's fluent API:**

```java
import static org.assertj.core.api.Assertions.*;

@Test
void Test_should_UseAssertJ() {
    // Arrange
...
```java
@Test
void Test_should_ThrowException() {
...
### 8. Use ArgumentCaptor to Verify Arguments
...
@Test
void Test_should_SaveUser_withCorrectData() {
    // Arrange
...
### 9. Use @BeforeEach for Shared Setup
...
❌ **DO NOT:**
- Use Spring context (`@SpringBootTest`) — pure unit tests only
- Test private methods directly — test via public interface
- Write integration tests
- Use Hamcrest — use AssertJ only
- Leave placeholder comments like `// add more tests`

✅ **DO:**
- Keep tests fast and isolated
- Mock all external dependencies (use `@Mock`, `@MockBean`)
- Test via public interface only
...
#### **2. SỬA: `openhands_cli/instructions/utgen/workflows.py`**

**Thêm Step 9 vào `OVERALL_WORKFLOW`:**
...
- Rename tests to: `test_should_[result]_when_[condition]`
- Use `@pytest.fixture` for reusable setup
- Use `@pytest.mark.parametrize` for data-driven tests
- Use class-based grouping for related scenarios
- Move shared fixtures to `conftest.py`
- Use MagicMock/patch or pytest-mock for mocking
- Keep each test focused on ONE behavior
- Add docstrings for complex tests
- Remove unused imports and placeholder comments
...
- Do NOT use `@SpringBootTest` — pure unit tests only
- Use AssertJ only (no Hamcrest)
- Remove unused imports and placeholder comments

**IMPORTANT:** This step is REQUIRED — do not skip refactoring.
"""
```

**Cập nhật `PYTHON_WORKFLOW` và `JAVA_WORKFLOW`:**

```python
PYTHON_WORKFLOW = """
...

7. **Execute Keploy Gen Command**

8. **Refactor Generated Tests (REQUIRED):**
    After Keploy generates tests, refactor to ensure:
    - ✅ AAA pattern (Arrange-Act-Assert)
    - ✅ Naming: `test_should_[result]_when_[condition]`
    - ✅ pytest fixtures for reusable setup
    - ✅ @pytest.mark.parametrize for data-driven tests
    - ✅ Class-based grouping for related scenarios
    - ✅ Shared fixtures in conftest.py
    - ✅ Proper mocking (MagicMock/patch or pytest-mock)
    - ✅ One test = one behavior
    - ✅ Descriptive docstrings for complex tests
    - ✅ No unused imports or placeholder comments
    
    See `openhands_cli/instructions/utgen/refactor_guides.py` for detailed guidelines.
"""
...
#### **3. SỬA: `openhands_cli/instructions/dev_skills.py`**
...
**Cập nhật `KEPLOY_GEN_UNIT_TEST` content:**
...
#### **4. SỬA: `openhands_cli/instructions/utgen/__init__.py`**

**Thêm export refactor guides:**

```python
"""
UTGen - Unit Test Generation module using Keploy.

This module provides:
- constants.py: Configuration and defaults
- workflows.py: Workflow documentation
- refactor_guides.py: Refactoring guidelines for generated tests
- error_handling.py: Common issues and solutions
- scripts/: Script templates for Python and Java
"""
...
## ✅ Checklist Thực Hiện (Cập Nhật)

| Step | File | Action | Status |
|------|------|--------|--------|
| 1 | `utgen/refactor_guides.py` | TẠO MỚI - Chứa PYTHON_REFACTOR_GUIDE và JAVA_REFACTOR_GUIDE | ⏳ |
| 2 | `utgen/workflows.py` | SỬA - Thêm Step 9 vào OVERALL_WORKFLOW | ⏳ |
| 3 | `utgen/workflows.py` | SỬA - Thêm bước refactor vào PYTHON_WORKFLOW và JAVA_WORKFLOW | ⏳ |
| 4 | `dev_skills.py` | SỬA - Thêm import refactor_guides | ⏳ |
| 5 | `dev_skills.py` | SỬA - Thêm refactor guides vào KEPLOY_GEN_UNIT_TEST content | ⏳ |
| 6 | `utgen/__init__.py` | SỬA - Export refactor guides | ⏳ |
...
### **Trước:**
- ❌ Keploy generate tests → Agent dừng lại
- ❌ Tests có thể không clean, không theo chuẩn
- ❌ Không có hướng dẫn refactor

### **Sau:**
- ✅ Keploy generate tests → Agent refactor bắt buộc
- ✅ Tests clean, rõ ràng, theo chuẩn AAA
- ✅ Có hướng dẫn chi tiết cho Python và Java
- ✅ Agent biết cách refactor đúng best practices
...
- [ ] ✅ File `refactor_guides.py` được tạo với đầy đủ guidelines
- [ ] ✅ OVERALL_WORKFLOW có Step 9: Refactor Generated Tests
- [ ] ✅ PYTHON_WORKFLOW và JAVA_WORKFLOW có bước refactor
- [ ] ✅ KEPLOY_GEN_UNIT_TEST content bao gồm refactor guides
- [ ] ✅ Agent refactor tests sau khi Keploy generate
- [ ] ✅ Tests tuân thủ AAA pattern và naming conventions
- [ ] ✅ Không còn placeholder comments hoặc unused imports
...
**Dựa trên:** `plan/refactor_utgen_skill.md`  
**Ngôn ngữ:** Tiếng Việt  
**Version:** 3.0 (Refactor guides + Best practices)
