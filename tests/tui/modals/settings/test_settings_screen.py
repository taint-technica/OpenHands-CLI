...
def test_is_initial_setup_required(
    ...
    fake_agent_store.save(
        _make_agent(model="openai/gpt-4o", api_key="k", base_url=None)
    )
...
async def test_dependency_chain_enables_fields_when_fetched_models_present(app):
    ...
    screen.proxy_url_input.value = (
        screen.proxy_url_input.value or "http://localhost:4000"
    )
